...
| Same blanks return **after suspend/resume** | Two causes: (1) CUDA context loss — the wake hook restarts the daemon to re-init it; (2) the PipeWire capture node comes back muted/suspended/wrong-gain. The wake hook now also runs `pactl resync` on wake. If blanks persist, check the mic isn't `SUSPENDED` (`pactl list sources`) and re-set its volume. |
